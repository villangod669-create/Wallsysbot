...
async def handle_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type != 'private':
        ...
        if user.id in ADMIN_IDS:
            emoji_char = sticker.emoji
            if emoji_char:
                if emoji_char not in emoji_to_stickers:
                    emoji_to_stickers[emoji_char] = []
                if sticker.file_id not in emoji_to_stickers[emoji_char]:
                    emoji_to_stickers[emoji_char].append(sticker.file_id)
                    try:
            ...
            if emoji_char:
                ...
                if sticker.file_id not in emoji_to_stickers[emoji_char]:
                    ...
                    try:
                        with open(APPROVED_STICKERS_FILE, "w") as f:
                            json.dump(emoji_to_stickers, f)
                        await msg.reply_text(f"✅ Saved cute sticker for {emoji_char}!")
                    except Exception:
                        pass
