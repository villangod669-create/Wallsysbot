...
WALLSY_PERSONA = (
    ...
    "6. SENDING STICKERS: Stickers are just a fun, interactive way to communicate like big emojis. You can send any sticker based on the conversation context to make the chat fun! You don't have to send one in every single reply, but use them naturally. When you do send one, include the exact text [STICKER:emoji] where emoji is a relevant single emoji.\n"
...
async def process_user_buffer(user_id, update, context):
    ...
    if any("[User communicated using a sticker" in m for m in messages):
        user_text += "\n\n(System Note: The user communicated using a sticker. Do NOT describe the sticker visually or say 'nice sticker', just respond naturally to the emotion/meaning! You can optionally send a sticker back if you want.)"
...
async def handle_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Prevent bot from echoing this exact sticker back immediately
    if user.id not in user_sticker_history:
        user_sticker_history[user.id] = []
    user_sticker_history[user.id].append(sticker.file_id)
    if len(user_sticker_history[user.id]) > 10:
        user_sticker_history[user.id].pop(0)
    ...
    if is_edited:
        user_buffers[user.id]["messages"].append(f"[User EDITED their message to communicate using a sticker conveying: {description}]")
    else:
        user_buffers[user.id]["messages"].append(f"[User communicated using a sticker conveying: {description}]")
